...
def crawStEz(amenity,page_num):
    ...
    url='http://www.streeteasy.com/buildings/downtown/building_amenities:%s?page=%s'%(amenity,page_num)
    print 'url: %s'%url
...
def runforsix(amenity):
    ...
    con = crawStEz(amenity,'1')
    nextpageN = procHtml(con)
    p = 1
    t = 0
    while nextpageN != 'last page':
    #while nextpageN != 3:
        con = crawStEz(amenity, nextpageN)
        nextpageN = procHtml(con)
        time.sleep(120)
        p=+1
        if p % 10 ==0:
...
if __name__=="__main__":
    ...
    if len(sys.argv)!=2:
        print 'should be:'+sys.argv[0]+' <amenity: leed_registration, doorman, gym, laundry, pool, parking>'
        sys.exit(1)
